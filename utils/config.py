...
class ProviderConfig:
	...
	def from_dict(cls, name: str, data: dict, *, defaults: 'ProviderConfig | None' = None) -> 'ProviderConfig':
		...
		bypass_method = defaults.bypass_method if defaults else None
		if data.get('signin_method') == 'browser_waf':
			bypass_method = 'waf_cookies'
		elif data.get('signin_method') == 'http_login':
			bypass_method = None
		if 'bypass_method' in data:
			bypass_method = data['bypass_method']
		return cls(
			name=name,
			domain=data.get('domain', defaults.domain) if defaults else data['domain'],
	...
	def load_from_env(cls) -> 'AppConfig':
		...
		providers = {
			...
			'agentrouter': ProviderConfig(
				...
				login_path='/login',
				sign_in_path=None,  # 每日通过 GitHub OAuth 登录触发奖励
				user_info_path='/api/user/self',
				api_user_key='new-api-user',
				bypass_method='waf_cookies',
...
def load_accounts_config() -> list[AccountConfig] | None:
	...
	try:
		...
		for i, account_dict in enumerate(accounts_data):
			...
			if not account_dict.get('api_user'):
