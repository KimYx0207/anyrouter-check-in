...
- 补充 AgentRouter 云端访问验证的诊断与代理配置说明；HTTP 200 的验证页面不会视为签到成功。
- 区分用户信息接口的登录失效与 HTML、防护页面或无效 JSON 响应，保留 HTTP 状态和响应类型。
- 调试模式下保存用户信息获取失败时的页面截图，便于定位 AgentRouter 在云端的站点验证问题；不输出 Cookie 或响应正文。
...
- 🐛 **修复冷却通知签到时间**
  - 当前 24 小时收益周期已过期时，不再展示旧的“签到成功时间”
  - 避免历史正收益记录长期显示为当前周期时间，例如一直显示 2026/04/18
- 🔧 **优化冷却通知格式**
  - 没有当前周期收益时，不再显示 `+$0.0`
  - 仅在存在有效收益或有效签到时间时显示收益括号
...
**原因**：站点可能按浏览器状态或运行网络触发访问验证，session 有效时也可能出现。

**排查方法**：

- HTTP 401 表示登录凭据失效或账号不匹配，按上面的步骤重新登录并更新 session 和 api_user。
- HTTP 200 但响应为 HTML 或无效 JSON，不代表签到成功。手动运行 Actions 时启用 `debug`，查看上传的页面截图，确认是否要求站点验证。
- 如果同一账号在本机成功、GitHub Actions 中遇到访问验证，可在仓库的 `production` 环境配置 `PROXY_SUBSCRIPTION_URL` Secret，使用已有的 Clash/mihomo 兼容订阅。工作流会启动代理并将地址传给签到程序。
- 对应 provider 还需设置 `use_proxy: true`；AgentRouter 默认已启用。该选项需要实际可用的代理配置，本机的 `127.0.0.1` 地址不能直接作为云端代理。
- 本机运行可通过 `CHECKIN_PROXY_URL` 配置现有 HTTP/SOCKS 代理。代理可用后仍须检查平台认证响应和签到结果。
