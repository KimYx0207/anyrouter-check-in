...
- 验证分支增加手动 AgentRouter 请求对照，检查网页请求头、XHR 与接口地址上的站点验证；仅输出认证结果，不输出凭据或响应正文。
- 在首次打开站点前载入账号 session，使用同一份会话完成站点验证、控制台访问和签到请求。
- AgentRouter 显示阿里云滑块时按当前页面完成交互，兼容新旧控件并用用户信息接口确认结果；验证未通过仍报告失败。
